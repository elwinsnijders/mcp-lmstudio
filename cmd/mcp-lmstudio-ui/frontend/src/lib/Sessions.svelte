<script>
  ...
  $: projectSections = buildProjectView(sessions, groupMap, filter)
  ...
  function buildItemsForSessions(sessionList, groupMap, lc) {
    const groupSessions = {}
    const ungrouped = []
  ...
  function buildProjectView(sessions, groupMap, filter) {
    ...
    for (const s of sessions) {
      ...
      if (s.project) {
        if (!projectMap[s.project]) projectMap[s.project] = []
        projectMap[s.project].push(s)
      } else {
        noProject.push(s)
      }
    }
    ...
    const projectNames = Object.keys(projectMap).sort((a, b) => {
      const latestA = projectMap[a].reduce((max, s) => s.lastActiveAt > max ? s.lastActiveAt : max, '')
      const latestB = projectMap[b].reduce((max, s) => s.lastActiveAt > max ? s.lastActiveAt : max, '')
      return latestB.localeCompare(latestA)
    })

    for (const name of projectNames) {
      const projectSessions = projectMap[name]
      ...
      sections.push({
        ...
        items: buildItemsForSessions(projectSessions, groupMap, lc),
      })
    }
    ...
    if (noProject.length > 0) {
      sections.push({
        ...
        activeCount: noProject.filter(s => s.status === 'active').length,
        items: buildItemsForSessions(noProject, groupMap, lc),
      })
    }
...
<div class="flex flex-col h-[calc(100vh-4rem)]">
  ...
  <div class="mb-3 shrink-0">
    <input
      type="text"
      placeholder="Filter by ID, task, profile, model, status, group, project..."
      bind:value={filter}
      ...
      {:else}
        {#each projectSections as section}
          {#if section.type === 'project'}
            ...
            >
              <svg class="w-3.5 h-3.5 text-violet-400 transition-transform {isProjectExpanded ? 'rotate-90' : ''}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
              ...
              <span class="text-sm font-semibold text-violet-800">{section.name}</span>
              {#if section.activeCount > 0}
                <span class="inline-flex px-1.5 py-0.5 rounded border text-[10px] font-semibold text-emerald-700 bg-emerald-50 border-emerald-200">
            ...
            {#if isProjectExpanded}
              {#each section.items as item}
                {#if item.type === 'group'}
                  ...
                  {#if isExpanded}
                    {#each item.sessions as session (session.id)}
                      <div class="px-4 py-2.5 grid grid-cols-[120px_70px_100px_1fr_110px_90px_90px] gap-2 items-center border-b border-gray-50 hover:bg-gray-50/70 transition-colors pl-16">
                ...
                {:else}
                  ...
                  <div class="px-4 py-3 grid grid-cols-[120px_70px_100px_1fr_110px_90px_90px] gap-2 items-center border-b border-gray-50 hover:bg-gray-50/70 transition-colors pl-10">
          ...
          {:else}
            <!-- no-project section: render items without a project header -->
            {#each section.items as item}
              {#if item.type === 'group'}
                {@const g = item.group}
                {@const isExpanded = expandedGroups[g.id] !== false}
